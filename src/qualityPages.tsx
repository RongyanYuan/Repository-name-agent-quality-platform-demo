...
export function OverviewPage() {
  ...
  return <div className="space-y-5">
    <PageIntro eyebrow="QUALITY OVERVIEW" title="质量总览" description="从用户满意度出发，分别判断产物是否合格、执行过程是否符合业务预期，再下钻到首个关键失败。" meta={<><span className="font-medium text-ink">{getScopeLabel(state.filters)}</span> · {totalProducts} 个产物 · 所有聚合信号均可回到 Task / Trace</>} actions={<><Button icon={ListFilter} onClick={() => navigateContext('/tasks', { params: { status: 'FAIL' } })}>查看异常任务</Button><Button variant="primary" icon={ArrowRight} onClick={() => navigateContext('/benchmarks')}>版本回归</Button></>} />

    <Panel labelledBy="north-star-title" className="overflow-hidden border-accent/30">
      <SectionHeader id="north-star-title" eyebrow="NORTH STAR" title="用户满意度" description="最终被用户接受的合格产物 / 全部产物。用户行为信号保留来源 Event，可直接回到对应 Task / Trace。" actions={<StatusBadge status="北极星指标" tone="info" showIcon={false} />} />
      <div className="mt-4 grid min-w-0 grid-cols-1 gap-3 xl:grid-cols-[minmax(280px,0.85fr)_minmax(0,2.15fr)]">
        <OverviewMetricCard id={satisfaction.id} label={satisfaction.label} value={satisfaction.value} trend={satisfaction.trend} numerator={satisfaction.numerator} denominator={satisfaction.denominator} description="合格且最终被接受的产物占全部产物的比例" tone="info" featured onClick={() => openMetric(satisfaction.id)} />
    ...
    <Panel labelledBy="module-diagnostics-title">
      <SectionHeader id="module-diagnostics-title" eyebrow="MODULE DIAGNOSTICS" title="模块诊断与首错归因" description="模块状态用于解释总指标，不作为新的质量总分。Context 和 Memory 只展示节点状态与证据，不作为一期正式 KPI。" actions={<OutcomeLegend />} />
      ...
      <div className="mt-5 border-t border-line pt-4">
        <SectionHeader level={3} eyebrow="FIRST FAILURE" title="首个关键失败分布" description="每条 Trace 只计入一个首个非派生失败；下游 DERIVED_FAIL 保留证据但不重复计数。" />
...
export function PerformancePage() {
  ...
  return <div className="space-y-5">
    <PageIntro eyebrow="PERFORMANCE MONITORING" title="性能监控" description="观察 Agent 是否变慢、变贵、变得不稳定，并把异常直接带回 Trace。" meta={<><span className="font-medium text-ink">{getScopeLabel(state.filters)}</span> · 复杂度来自 Eval Agent 判断</>} actions={<Button icon={ArrowRight} onClick={() => navigateContext('/tasks', { params: { status: 'FAIL' } })}>查看异常 Trace</Button>} />
    <Panel className="border-accent/25">
      <SectionHeader eyebrow="V2 RAW SIGNALS" title="效率、成本与稳定性" description="新口径保留原始信号；缺少 TTFT 或基线时明确显示 UNKNOWN，不推断通过。" />
      <div className="mt-4 grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MiniMetric icon={Gauge} label="Total Latency Efficiency" value={performance.totalLatencyEfficiency.status === 'UNKNOWN' ? 'UNKNOWN' : `${performance.totalLatencyEfficiency.rate.toFixed(1)}%`} tone={performance.totalLatencyEfficiency.status === 'FAIL' ? 'warn' : performance.totalLatencyEfficiency.status === 'PASS' ? 'pass' : 'neutral'} note="按业务预期时延达标" />
        ...
        <MiniMetric label="Throughput" value={`${performance.throughput.tokensPerSecond.toLocaleString()} tok/s`} note="Input + Output / 总时延" />
        <MiniMetric label="Cost baseline" value={performance.cost.deviation === null ? 'UNKNOWN' : `${performance.cost.deviation > 0 ? '+' : ''}${performance.cost.deviation}%`} tone={performance.cost.deviation !== null && performance.cost.deviation > 0 ? 'warn' : 'neutral'} note={`仅合格产物 ${performance.cost.evaluatedProducts} 个进入分母`} />
      </div>
      <div className="mt-3 grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <MiniMetric label="Input / Output Token" value={`${performance.tokens.input.toLocaleString()} / ${performance.tokens.output.toLocaleString()}`} note={`Total ${performance.tokens.total.toLocaleString()}`} />
        <MiniMetric label="Cache hit" value={`${performance.cache.hitRate.toFixed(1)}%`} tone={performance.cache.hitRate > 0 ? 'pass' : 'neutral'} note={`${performance.cache.hits} hits · ${performance.cache.misses} misses`} />
        <MiniMetric label="Tool frequency" value={`${performance.toolFrequency.perTask.toFixed(2)} / task`} note={`${performance.toolFrequency.calls} calls · ${performance.toolFrequency.perSecond.toFixed(2)} / sec`} />
        <MiniMetric label="One-shot Tool success" value={performance.oneShotSuccess.eligible ? `${performance.oneShotSuccess.rate.toFixed(1)}%` : 'N/A'} tone={performance.oneShotSuccess.rate > 80 ? 'pass' : 'warn'} note={`${performance.oneShotSuccess.successes}/${performance.oneShotSuccess.eligible}，不含 Retry/Recovery`} />
      </div>
      <div className="mt-4 grid min-w-0 gap-3 lg:grid-cols-2">
        <div className="rounded-md border border-line bg-slate-50/60 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-semibold text-ink">Latency efficiency bands</p><span className="text-[10px] text-muted">实际 / 预期</span></div><div className="mt-3 grid grid-cols-2 gap-2">{performance.latencyBands.map((band) => <button key={band.band} type="button" onClick={() => navigateContext('/tasks', { params: { metric: 'latency-efficiency', latencyBand: band.band } })} className="rounded border border-line bg-white p-2 text-left hover:border-accent/50"><p className="text-[11px] font-medium text-ink">{band.band}</p><p className="mt-1 text-lg font-semibold text-ink">{band.count}</p><p className="text-[10px] text-muted">{band.rate.toFixed(1)}%</p></button>)}</div></div>
        <div className="rounded-md border border-line bg-slate-50/60 p-3"><p className="text-xs font-semibold text-ink">Risk / Commercial interception</p><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" onClick={() => navigateContext('/tasks', { params: { metric: 'risk-interception' } })} className="rounded border border-fail/20 bg-fail/5 p-2 text-left"><p className="text-[11px] text-muted">Risk blocks</p><p className="mt-1 text-lg font-semibold text-fail">{performance.risk.blocked}</p><p className="text-[10px] text-muted">{performance.risk.rate.toFixed(1)}% · {Object.keys(performance.risk.reasons).length} reasons</p></button><button type="button" onClick={() => navigateContext('/tasks', { params: { metric: 'commercial-interception' } })} className="rounded border border-warn/20 bg-warn/5 p-2 text-left"><p className="text-[11px] text-muted">Commercial blocks</p><p className="mt-1 text-lg font-semibold text-warn">{performance.commercial.blocked}</p><p className="text-[10px] text-muted">{performance.commercial.rate.toFixed(1)}% · {Object.keys(performance.commercial.reasons).length} reasons</p></button></div><div className="mt-3 flex flex-wrap gap-1.5 text-[10px] text-muted">{[...Object.entries(performance.risk.reasons), ...Object.entries(performance.commercial.reasons)].map(([reason, count]) => <span key={reason} className="rounded bg-white px-2 py-1">{reason} · {count}</span>)}</div></div>
      </div>
    </Panel>
    <div className="grid grid-cols-2 gap-3 md:grid-cols-4"><MiniMetric icon={Clock3} label="P50 Latency" value={formatDuration(latencies.sort((a, b) => a - b)[Math.floor(latencies.length * 0.5)] ?? 0)} note="兼容视图" /><MiniMetric icon={Clock3} label="P90 Latency" value={formatDuration(latencies[Math.floor(latencies.length * 0.9)] ?? 0)} note="兼容视图" /><MiniMetric icon={Timer} label="P95 Latency" value={formatDuration(latencies[Math.floor(latencies.length * 0.95)] ?? 0)} tone="warn" note="异常点可下钻" /><MiniMetric icon={Gauge} label="Task Cost" value={formatCurrency(tasks.reduce((sum, task) => sum + task.cost, 0))} note="Mock session total" /></div>
    <div className="grid grid-cols-1 gap-4 2xl:grid-cols-[minmax(0,1.15fr)_minmax(0,1fr)]"><Panel><SectionHeader eyebrow="TASK PERFORMANCE" title="复杂度 × 实际时延" description="不使用硬阈值，保留 Eval Agent 复杂度判断和真实观察时延。" /><div className="mt-4"><BarChartView data={complexityData} labelKey="label" valueKey="median" height={250} valueFormatter={formatDuration} /><div className="mt-3 grid grid-cols-3 gap-2">{complexityData.map((item) => <MiniMetric key={item.label} label={item.label} value={item.value} note={`median ${formatDuration(item.median)}`} />)}</div></div></Panel><Panel><SectionHeader eyebrow="EXECUTION VOLUME" title="调用与成本" description="用量信号与质量信号分栏呈现。" /><div className="mt-4 grid grid-cols-2 gap-3"><MiniMetric label="Token Usage" value={tasks.reduce((sum, task) => sum + task.tokens, 0).toLocaleString()} note="累计 Token" /><MiniMetric label="Model Calls" value={tasks.reduce((sum, task) => sum + task.modelCalls, 0)} note="模型调用" /><MiniMetric label="Tool Calls" value={tasks.reduce((sum, task) => sum + task.toolCalls, 0)} note="能力调用" /><MiniMetric label="Loop / Retry" value={`${tasks.reduce((sum, task) => sum + task.loopCount, 0)} / ${tasks.reduce((sum, task) => sum + task.retryCount, 0)}`} tone="warn" note="冗余与恢复" /></div></Panel></div>
    <Panel><SectionHeader eyebrow="TOOL PERFORMANCE" title="Tool baseline deviation" description="当前 P95 超过 rolling baseline 的 Tool 标记 Warning；点击行查看异常 Trace。" /><div className="mt-4 overflow-x-auto"><table className="data-table"><thead><tr><th>Tool</th><th>Calls</th><th>Failure</th><th>P50</th><th>P95</th><th>7d baseline</th><th>14d baseline</th><th>Deviation</th><th /></tr></thead><tbody>{toolMetrics.map((metric) => <tr key={metric.tool} className="cursor-pointer" onClick={() => setToolDrawer(metric.tool)}><td className="font-medium text-ink">{metric.tool}</td><td>{metric.calls}</td><td><StatusBadge status={`${metric.failureRate.toFixed(1)}%`} tone={metric.failed ? 'fail' : 'pass'} /></td><td>{formatDuration(metric.p50)}</td><td>{formatDuration(metric.p95)}</td><td>{formatDuration(metric.baseline7)}</td><td>{formatDuration(metric.baseline14)}</td><td><StatusBadge status={`${metric.deviation > 0 ? '+' : ''}${metric.deviation}%`} tone={metric.warning ? 'warn' : 'pass'} /></td><td><ChevronRight className="h-4 w-4 text-slate-400" aria-hidden="true" /></td></tr>)}</tbody></table></div></Panel>
    <Panel><SectionHeader eyebrow="MODEL PERFORMANCE" title="Model calls" description="模型版本、Token、平均时延和失败/超时独立展示。" /><div className="mt-4 overflow-x-auto"><table className="data-table"><thead><tr><th>Model</th><th>Calls</th><th>Tokens</th><th>Avg latency</th><th>Fail</th><th>Timeout</th><th>Unnecessary call</th></tr></thead><tbody>{modelMetrics.map((metric) => <tr key={metric.model}><td className="font-medium">{metric.model}</td><td>{metric.calls}</td><td>{metric.tokens.toLocaleString()}</td><td>{formatDuration(metric.latency)}</td><td className={metric.failures ? 'text-fail' : 'text-pass'}>{metric.failures}</td><td>{metric.timeouts}</td><td>{metric.unnecessaryRate.toFixed(1)}%</td></tr>)}</tbody></table></div></Panel>
...
function TraceTimeline({ taskId, onObservation }: { taskId: string; onObservation: (observation: Observation) => void }) {
  ...
  if (!task || !trace) return <EmptyState title="Trace unavailable" description="No trace is attached to this task." />
...
function EvalRow({ evaluation, onEvidence, onOverride }: { evaluation: EvalResult; onEvidence: (id: string) => void; onOverride: (evaluation: EvalResult) => void }) {
...
function TaskDetail({ task, onClose }: { task: Task; onClose: () => void }) {
  ...
  return <><Drawer open={Boolean(task)} onClose={onClose} width="lg" title={`Task ${shortId(task.id)}`} description={`${task.traceId} · ${task.sessionId} · ${humanDate(task.timestamp)}`} footer={<div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><StatusBadge status={task.status} /><StatusBadge status={task.rootCause === 'None' ? 'No root cause' : task.rootCause} tone={rootCauseTone(task.rootCause)} showIcon={false} /></div>{caseRecord && <Button variant={caseRecord.status === 'Confirmed Badcase' ? 'secondary' : 'danger'} icon={ShieldCheck} onClick={confirmCase}>{caseRecord.status === 'Confirmed Badcase' ? '已确认 Badcase' : '确认 Badcase'}</Button>}</div>}><div className="space-y-5"><div className="grid grid-cols-2 gap-2 md:grid-cols-4"><MiniMetric label="Business" value={task.businessType} /><MiniMetric label="Outcome Type" value={task.outcomeType} /><MiniMetric label="Complexity" value={task.complexity} /><MiniMetric label="Latency" value={formatDuration(task.latency)} tone={task.latency > 7000 ? 'warn' : 'neutral'} /></div><Panel><SectionHeader level={3} eyebrow="TASK INPUT" title="原始 Query" /><p className="mt-3 rounded-md bg-slate-50 p-3 text-sm leading-6 text-ink">{task.query}</p><div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-[11px] text-muted md:grid-cols-4"><span><b className="text-slate-400">Agent</b><br />{task.agentVersion}</span><span><b className="text-slate-400">Environment</b><br />{task.environment}</span><span><b className="text-slate-400">Skill</b><br />{task.skill}</span><span><b className="text-slate-400">Final outcome</b><br />{task.finalOutcome}</span></div></Panel><Panel><SectionHeader level={3} eyebrow="RESULT EVAL" title="结果评测" description="自动评测与人工标签并排保留。" /><div className="mt-3 grid gap-2">{task.evals.map((evaluation) => <EvalRow key={evaluation.id} evaluation={evaluation} onEvidence={(evidenceId) => { const item = state.evidence.find((candidate) => candidate.id === evidenceId); const obs = state.observations.find((candidate) => candidate.id === item?.observationId); setObservation(obs) }} onOverride={(evaluation) => { setOverrideTarget(evaluation); setOverrideStatus(effectiveEvalStatus(evaluation) === 'FAIL' ? 'FAIL' : 'PASS'); setOverrideReason(evaluation.humanReason ?? '人工复核确认。') }} />)}</div></Panel><Panel><SectionHeader level={3} eyebrow="TRACE TIMELINE" title="执行路径与首个失败" description="点击节点打开 Observation Details。" /><div className="mt-3"><SharedTraceTimeline trace={trace} evidence={state.evidence} onObservationClick={setObservation} onEvidenceClick={(evidenceItem, observationItem) => { setObservation(observationItem ?? state.observations.find((candidate) => candidate.id === evidenceItem.observationId)) }} /></div></Panel></div></Drawer><ObservationDetails observation={observation} onClose={() => setObservation(undefined)} /><Modal open={Boolean(overrideTarget)} onClose={() => setOverrideTarget(undefined)} title={`人工 Override · ${overrideTarget?.dimension ?? ''}`} description="自动结果不会被删除；人工结论会作为当前有效标签。" footer={<div className="flex justify-end gap-2"><Button onClick={() => setOverrideTarget(undefined)}>取消</Button><Button variant="primary" onClick={saveOverride}>保存 Override</Button></div>}><div className="space-y-4"><Field label="人工结论" required><SelectInput value={overrideStatus} onChange={(event) => setOverrideStatus(event.target.value as 'PASS' | 'FAIL')}><option value="PASS">PASS</option><option value="FAIL">FAIL</option></SelectInput></Field><Field label="人工 Note" required><TextArea value={overrideReason} onChange={(event) => setOverrideReason(event.target.value)} /></Field><InlineNotice tone="info">Auto Eval: {overrideTarget?.autoStatus} · {overrideTarget?.autoReason}</InlineNotice></div></Modal></>
}
...
export function TasksPage() {
  ...
  const controls: FilterControl[] = [
    { id: 'status', label: 'Status', value: state.filters.status ?? '', options: [{ value: 'PASS', label: 'PASS' }, { value: 'FAIL', label: 'FAIL' }, ...(['Effective', 'Effective but Inefficient', 'Failed'] as const).map((value) => ({ value, label: value }))], onChange: (value) => dispatch({ type: 'SET_FILTERS', filters: { status: (value || undefined) as FilterState['status'] } }) },
    { id: 'business', label: 'Business', value: state.filters.businessType === 'All' ? '' : state.filters.businessType, options: BUSINESS_TYPES.filter((value) => value !== 'All').map((value) => ({ value, label: value })), onChange: (value) => dispatch({ type: 'SET_FILTERS', filters: { businessType: (value || 'All') as FilterState['businessType'] } }) },
    { id: 'complexity', label: 'Complexity', value: state.filters.complexity ?? '', options: COMPLEXITIES.map((value) => ({ value, label: value })), onChange: (value) => dispatch({ type: 'SET_FILTERS', filters: { complexity: (value || undefined) as FilterState['complexity'] } }) },
    ...
    { id: 'badcase', label: 'Badcase', value: state.filters.badcase ?? '', options: [{ value: 'yes', label: 'Yes' }, { value: 'no', label: 'No' }], onChange: (value) => dispatch({ type: 'SET_FILTERS', filters: { badcase: (value || undefined) as FilterState['badcase'] } }) }
  ]
  const reset = () => dispatch({ type: 'RESET_FILTERS' })
  const openTask = (selected: Task) => navigateContext('/tasks', { params: { taskId: selected.id } })
  return <div className="space-y-4"><PageIntro eyebrow="TASK / TRACE WORKBENCH" title="Task / Trace" description="从任务结果、Eval Evidence 到逐节点 Trace 的治理工作台。" meta={<><span className="font-medium text-ink">{tasks.length}</span> 条记录 · 搜索和筛选状态会保留在 URL</>} actions={<Button variant="secondary" icon={FilterX} onClick={reset}>清除筛选</Button>} /><FilterBar title="Task filters" controls={controls} searchValue={state.filters.search} onSearchChange={(value) => dispatch({ type: 'SET_FILTERS', filters: { search: value } })} resultCount={tasks.length} onReset={reset} searchPlaceholder="搜索 task_id / trace_id / query" /><div className="flex flex-wrap items-center justify-between gap-2 text-[11px] text-muted"><OutcomeLegend /><span>{state.filters.metric ? `Metric filter: ${state.filters.metric}` : '可从 KPI / 异常点直接下钻'}</span></div><DataTable columns={taskColumns(openTask)} rows={tasks} rowKey={(row) => row.id} onRowClick={openTask} selectedRowId={taskId} empty={<EmptyState icon={Search} title="没有匹配的 Task / Trace" description="尝试清除某个筛选，或搜索 task_id、trace_id 和 query。" />} />{task && <TaskDetail task={task} onClose={() => navigateContext('/tasks', { clear: ['taskId', 'traceId'] })} />}</div>
}
