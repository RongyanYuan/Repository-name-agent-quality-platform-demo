...
describe('anomaly monitoring page', () => {
  it('renders the four monitoring sections and explicit unknown baseline copy', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Model 性能' })).toBeVisible()
    expect(screen.getByRole('heading', { name: '商业化与风控' })).toBeVisible()
    expect(screen.getByRole('button', { name: /TTFT 效率/ })).toBeVisible()
    expect(screen.getByText('输入成本偏离率')).toBeVisible()
    expect(screen.getByText('输出成本偏离率')).toBeVisible()
    expect(screen.getByText('7 日基线')).toBeVisible()
    expect(screen.getByText('14 日基线')).toBeVisible()
